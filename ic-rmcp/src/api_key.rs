...
impl ApiKeyState {
    ...
    pub fn hash_key(raw_key: &str) -> HashedApiKey {
        // Keys returned by `create_from_entropy` are lowercase hexadecimal.
        // Decode that representation before hashing so lookup hashes the same
        // bytes that were used when the key was issued. For callers that use a
        // non-hex token, retain the conventional raw UTF-8 hashing behavior.
        match hex_decode(raw_key) {
            Some(bytes) => Sha256::digest(bytes).into(),
            None => Sha256::digest(raw_key.as_bytes()).into(),
        }
    }
...
mod tests {
    ...
    fn hash_key_decodes_issued_hex_key() {
        ...
        let digest = ApiKeyState::hash_key(&issued);
        assert_eq!(state.list_my_api_keys(alice)[0].hashed_key, digest);
    }
